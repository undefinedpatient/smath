...
TEST(CROSS_PRODUCT) {
    ...
    auto b01 = Vec3f{static_cast<float>(1/std::sqrt(2)),static_cast<float>(1/std::sqrt(2)),0};
...
TEST(BOOLEAN){
    ...
    assert_equal(v0.all(), false);
    assert_equal(v0.any(), true);
    assert_equal(v0.none(), false);

    Vec3f v1(0.0f,0.0f,0.0f);

    assert_equal(v1.all(), false);
    assert_equal(v1.any(), false);
    assert_equal(v1.none(), true);
}
